...
public class CompanyServiceImplementation {
	...
	static final Logger logger = Logger.getLogger(CompanyServiceImplementation.class);
		...
		if (org.apache.commons.lang.StringUtils.isEmpty(content)) {
			try {
				int count = ((Long) session
						.createQuery("select count(*) from com.synerzip.projectmanagementapp.model.Company")
						.uniqueResult()).intValue();
				if (count > 0) {
					Query query = session.createQuery("from com.synerzip.projectmanagementapp.model.Company");
	...
	public Company add(Company company) {
		...
		try {
			if (StringUtils.isEmptyOrWhitespaceOnly(company.getCompanyName())) {
				...
				throw new CanNotEmptyField("company contact number must be filled");
			} else {
			...
			throw new ConstraintViolationException("record already present with name-- " + company.getCompanyName(),
					null, null);
		} finally {
			session.close();
			logger.info("session closed successfully");
